...
export async function requestSandboxExecution(socketPath: string, spec: JobSpecV2, worktreePath: string, timeoutMs: number, modelAccess?: RunnerModelAccessGrantV2, agentPrompt?: string): Promise<SandboxExecutionResult> {
  ...
  return new Promise<SandboxExecutionResult>((resolvePromise, reject) => {
    ...
    socket.once("connect", () => socket.end(JSON.stringify({ spec, worktreePath, ...(modelAccess === undefined ? {} : { modelAccess }), ...(agentPrompt === undefined ? {} : { agentPrompt }) })));
